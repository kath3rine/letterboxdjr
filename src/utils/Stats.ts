...
export async function getStats(
    ...
    //const ratingCounts: {[rating: number]: number} = Object.assign({}, ...[1, 2, 3, 4, 5].map(key => ({ [key]: 0 })))
    const ratingCounts: {[rating: number]: number} = Object.assign({}, ...Array.from({ length: 5 }, (_, i) => ({
        [i + 1]: 0
      })));
    let totalRating = 0
    let rewatches = 0
